...
function sanitizeArgumentValue(val: mixed, arg_type: number): mixed {
  ...
  if (!(val instanceof Array) && (arg_type & ARG.List) === ARG.List) {
    ...
    if (typeof val === 'string') {
      return val.split(',').map((item: string) => ARG.cast(item.trim(), item_type));
    }
    return [val];
  }
 ...
 */
export async function validateAndFormatArguments(
  cmd_def: CMDDef,
  kwargs: {[string]: mixed},
  vmachine: VMachine,
  opts: EvalOptions,
  ...
  const args_infos: {[string]: ArgInfo} = Object.setPrototypeOf({}, null);
  for (const arg of cmd_def.args) {
    const info = getArgumentInfo(arg);
    if (info !== null) args_infos[info.names.long] = info;
  }

  // Normalize Names
  const in_arg_names = Object.keys(kwargs);
  let full_kwargs: {[string]: mixed} = Object.setPrototypeOf({}, null);
  ...
  for (const arg_name in args_infos) {
    ...
    if (!Object.hasOwn(full_kwargs, arg_name) && typeof arg_def.default_value !== 'undefined') {
  ...
  const new_kwargs: {[string]: mixed} = Object.setPrototypeOf({}, null);
