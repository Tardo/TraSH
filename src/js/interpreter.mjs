...
export type CMDCallbackInternal = (
  vmachine: VMachine,
  kwargs: {[string]: mixed},
  frame: Frame,
  opts: EvalOptions,
...
const OPERATOR_LEXER_TYPES: Map<string, number> = new Map([
  ...
  [SYMBOLS.QUESTION, LEXER.Question],
...
export default class Interpreter {
  #maskComments(data: string): string {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (quote) {
        ...
        quote = char;
      } else if (char === '/' && (i === 0 || /\s/.test(data[i - 1]))) {
        const next = data[i + 1];
        ...
        if (next === '/') {
          ...
          if (end === -1) throw new InvalidTokenError('/*', i, data.length);
          end += 2;
        }
        parts.push(data.slice(start, i), data.slice(i, end).replace(/[^\r\n]/g, ' '));
        start = end;
        i = end - 1;
      }
    }
    return parts.length ? parts.join('') + data.slice(start) : data;
  }
  ...
  tokenize(data: string, options: ParserOptions): Array<TokenInfo> {
    // Remove comments
    const clean_data = this.#maskComments(data);
      ...
      if (!is_escaped) {
        if (char === SYMBOLS.STRING || char === SYMBOLS.STRING_SIMPLE) {
          ...
          if (char === SYMBOLS.ARRAY_START) {
            ...
            if (
              (!isFalsy(options?.isData) && (char === delimiter || prev_char === delimiter)) ||
              char === SYMBOLS.DICTIONARY_SEPARATOR ||
              prev_char === SYMBOLS.DICTIONARY_SEPARATOR ||
              char === SYMBOLS.QUESTION ||
              prev_char === SYMBOLS.QUESTION
            ) {
              do_cut = true;
            } else if (
              char === SYMBOLS.EOC ||
              ...
              (char === SYMBOLS.ASSIGNMENT &&
                ...
                (prev_char === '' || !SYMBOLS_MATH_OPER_FIRST_CHARS.has(prev_char))) ||
              (char === SYMBOLS.NOT && prev_char !== SYMBOLS.NOT) ||
              // NOTE: '++' must stay a single token ('--' already accumulates thanks to the ARGUMENT guard)
              (SYMBOLS_MATH_OPER_FIRST_CHARS.has(char) &&
                (prev_char === '' || !SYMBOLS_MATH_OPER_COMPLEX_FIRST_CHARS.has(prev_char)) &&
                !value.startsWith(SYMBOLS.ARGUMENT) &&
                !(char === SYMBOLS.ADD && value === SYMBOLS.ADD)) ||
              ...
              (!SYMBOLS_MATH_OPER_FIRST_CHARS.has(char) &&
                SYMBOLS_MATH_OPER_SET.has(value) &&
                (isNumber(char) || char === SYMBOLS.VARIABLE)) ||
    ...
    if (options.ignoreErrors !== true && (in_string || in_array || in_block || in_lblock)) {
      const start = (options.offset ?? 0) + clean_data.length - value.length;
      throw new InvalidTokenError(value, start, (options.offset ?? 0) + clean_data.length);
    }
    if (value) {
  ...
  #lexer(
    token: string,
    next_char: string | void,
    prev_token_info: LexerInfo | void,
    prev_token_info_no_space: LexerInfo | void,
    ...
    if (!token_san) {
      ...
      prev_token_info &&
      prev_token_info[0] === LEXER.Space &&
      token_san[0] === SYMBOLS.ARGUMENT &&
      token_san !== SYMBOLS.DECREMENT &&
      ((token_san.length === 1 && typeof next_char === 'undefined') ||
        (token_san.length > 1 &&
          token_san[1] !== SYMBOLS.LOGIC_BLOCK_START &&
          token_san[1] !== SYMBOLS.ASSIGNMENT &&
          !isNumber(token_san[1])))
    ) {
    ...
    } else if (token_san === SYMBOLS.EOC || token_san === delimiter) {
      ttype = LEXER.Delimiter;
    } else if (token_san === SYMBOLS.DICTIONARY_SEPARATOR) {
      ttype = LEXER.Colon;
    } else if (
    ...
    } else if (token_san[0] === SYMBOLS.LOGIC_BLOCK_START && token_san.at(-1) === SYMBOLS.LOGIC_BLOCK_END) {
      token_san = token_san.substr(1, token_san.length - 2);
      ...
      if (
        (!prev_token_info ||
          (prev_token_info &&
            (prev_token_info[0] === LEXER.Space ||
              (!isNumber(prev_token_info[1]) &&
                prev_token_info[0] !== LEXER.Variable &&
                prev_token_info[0] !== LEXER.LogicBlock)))) &&
        next_char !== SYMBOLS.SPACE &&
        (next_char === SYMBOLS.VARIABLE ||
          next_char === SYMBOLS.LOGIC_BLOCK_START ||
          isNumber(next_char) ||
          (prev_token_info_no_space && LEXER_MATH_OPER_SET.has(prev_token_info_no_space[0])))
      ) {
      ...
      prev_token_info_no_space &&
      prev_token_info_no_space[0] !== LEXER.ArgumentShort &&
      prev_token_info_no_space[0] !== LEXER.ArgumentLong
    ) {
