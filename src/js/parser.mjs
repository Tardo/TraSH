...
export default class ASTParser {
  ...
  #parseUnit(tokens: Array<TokenInfo>, uopts: UnitOptions): ASTUnit {
    ...
    while (cursor.pos < tokens_len) {
      ...
      try {
        const stmt =
          uopts.isData === true ? this.#parseDataItem(unit, cursor) : this.#parseStatement(unit, cursor, uopts);
        if (stmt !== null) {
  ...
  #parsePrimary(unit: ASTUnit, cursor: {pos: number}): ASTNode {
    ...
    switch (token.type) {
      ...
      case LEXER.Array: {
        ...
        const itemCount = sub.statements.filter(st => st.expr).length;
        return {node: NODE.ArrayExpr, token, tokenIndex: ti, unit: sub, itemCount};
      }
      case LEXER.Dictionary: {
        ++cursor.pos;
        const sub = this.#subUnit(token.value, token.start + 1, {isData: true, silent: true});
        const itemCount = Math.floor(sub.statements.filter(st => st.expr).length / 2);
        return {node: NODE.DictExpr, token, tokenIndex: ti, unit: sub, itemCount};
      }
  ...
  #parseExpression(unit: ASTUnit, cursor: {pos: number}, min_bp: number, stop_on_assign?: boolean): ASTNode {
    ...
    // Ternary: cond ? cons : alt — lowest precedence, right-associative.
    // Only considered at the outermost call (min_bp === 0); binary-operator
    // RHS recursion uses min_bp > 0, so 'a + b ? c : d' still parses as
    // '(a + b) ? c : d', not 'a + (b ? c : d)'.
    if (min_bp === 0) {
      const q_token = unit.tokens[cursor.pos];
      if (typeof q_token !== 'undefined' && q_token.type === LEXER.Question) {
        ++cursor.pos;
        const consequent = this.#parseExpression(unit, cursor, 0);
        const c_token = unit.tokens[cursor.pos];
        if (typeof c_token === 'undefined' || c_token.type !== LEXER.Colon) {
          this.#throwToken(q_token);
        }
        ++cursor.pos;
        const alternate = this.#parseExpression(unit, cursor, 0);
        left = {
          node: NODE.Ternary,
          token: q_token,
          tokenIndex: this.#ti(unit, q_token),
          test: left,
          consequent: consequent.node === NODE.Missing ? null : consequent,
          alternate: alternate.node === NODE.Missing ? null : alternate,
        };
      }
    }
    return left;
  ...
  #parseFunctionParams(value: string): Array<ArgDef> {
    ...
    return value.split(',').map(item => {
      ...
      return [
        ...
        [varname, varname],
        false,
        i18n.t('cmdFuncTrash.args.name', 'The function parameter'),
        cdefault,
      ];
    });
  }
  ...
  #parseIf(unit: ASTUnit, cursor: {pos: number}): ASTNode {
    ...
    const readBranch = (): {check: ASTUnit, body: ASTUnit} => {
      ...
      const check = this.#subUnit(check_token.value, check_token.start + 1, {
        isData: false,
        silent: true,
        exprContext: true,
      });
  ...
  #parseFor(unit: ASTUnit, cursor: {pos: number}): ASTNode {
    ...
    if (typeof head_token === 'undefined' || !head_token.raw.trim().startsWith(SYMBOLS.FUNCTION_ARGS_START)) {
